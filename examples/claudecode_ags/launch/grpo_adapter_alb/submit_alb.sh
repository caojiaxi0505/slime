#!/usr/bin/env bash
# Apply / delete the Path A GRPO 1-node debug adapter Service + ALB Ingress.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
YAML="${SCRIPT_DIR}/service-ingress.yaml"
K8S_NAMESPACE="${K8S_NAMESPACE:-sn5-system-intern}"
NAME="${NAME:-jiaxicao-grpo-1node-debug-adapter}"

usage() {
  cat <<'EOF'
Usage: submit_alb.sh [--delete] [--dry-run] [--help]

Creates Service + internet-facing ALB Ingress for GRPO Master :9002.
...
for _ in $(seq 1 60); do
  ...
  if [[ -n "${ADDR}" ]]; then
    ...
    echo "  - Master pod must have labels app=cc-ags-recorder,workload=jiaxicao-grpo-1node-debug"
