#!/usr/bin/env bash
# Resume Hybrid from iter14 through iter24. Run the full continuation, but
# train only the first resumed assistant turn. Uses an independent ALB/tool/W&B run.
...
export JOB_NAME="${JOB_NAME:-jiaxicao-hybrid-i14-first-v2}"
export WORKLOAD_LABEL="${WORKLOAD_LABEL:-${JOB_NAME}}"
export INGRESS_NAME="${INGRESS_NAME:-${JOB_NAME}-adapter}"

export EXP_TAG="${EXP_TAG:-qwen35_9b_cc_ags_hybrid_i14_i24_firstturn_s1fix_agsretry_v2}"
...
export WANDB_RUN_ID="${WANDB_RUN_ID:-i8u61c8n}"
