...
Useful env:
  ...
  STEP_GRPO_FILTER         1
  WANDB_KEY                required for online WandB
...
STEP_GRPO_BRANCH_CONCURRENCY="${STEP_GRPO_BRANCH_CONCURRENCY:-8}"
...
WANDB_KEY="${WANDB_KEY:-}"
# Prefer hybrid EXP_TAG; do not inherit stale GRPO group from the shell env.
...
if [[ -z "${WANDB_KEY}" ]]; then
  echo "ERROR: WANDB_KEY is required (online WandB)." >&2
  exit 1
fi
...
export STEP_GRPO_HYBRID_K STEP_GRPO_BRANCH_CONCURRENCY STEP_GRPO_PPL_CLIP STEP_GRPO_FILTER
export STEP_GRPO_BUNDLE_DIR NUM_ROLLOUT SAVE_INTERVAL GLOBAL_BATCH_SIZE

RENDERED="$(mktemp)"
envsubst '${JOB_NAME} ${K8S_NAMESPACE} ${IMAGE_URI} ${SLIME_DIR} ${HF_CHECKPOINT} ${REF_MODEL_PATH} ${PROMPT_DATA} ${EVAL_DATA} ${EXP_TAG} ${LOG_DIR} ${PHASE} ${AGS_SECRET_NAME} ${SLIME_ADAPTER_PUBLIC_URL} ${WANDB_KEY} ${WANDB_PROJECT} ${WANDB_GROUP} ${WANDB_TEAM} ${STEP_GRPO_HYBRID_K} ${STEP_GRPO_BRANCH_CONCURRENCY} ${STEP_GRPO_PPL_CLIP} ${STEP_GRPO_FILTER} ${STEP_GRPO_BUNDLE_DIR} ${NUM_ROLLOUT} ${SAVE_INTERVAL} ${GLOBAL_BATCH_SIZE}' \
  < "${TEMPLATE}" > "${RENDERED}"
...
echo "    NUM_ROLLOUT=${NUM_ROLLOUT} SAVE_INTERVAL=${SAVE_INTERVAL} GLOBAL_BATCH_SIZE=${GLOBAL_BATCH_SIZE}"
echo "    hybrid K=${STEP_GRPO_HYBRID_K} filter=${STEP_GRPO_FILTER} branch_conc=${STEP_GRPO_BRANCH_CONCURRENCY}"
