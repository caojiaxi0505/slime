#!/usr/bin/env bash
# Render + apply / delete the Path A hybrid step-GRPO 2-node / 16-GPU hybrid PyTorchJob.
set -euo pipefail
...
Creates a 2×8GPU PyTorchJob (Master + Worker) for Path A hybrid step-GRPO.
Master labels match hybrid_adapter_alb Service (app=cc-ags-recorder,
workload=jiaxicao-hybrid-1node-debug).

Useful env:
  JOB_NAME                 jiaxicao-hybrid-2node-full
  ...
  EXP_TAG / LOG_DIR        default qwen35_9b_cc_ags_2node_hybrid_full
  ...
  ROLLOUT_BATCH_SIZE       default 16 (match naive GRPO RBS)
  ...
  WANDB_FORK_FROM          optional W&B fork point: RUN_ID?_step=N
...
JOB_NAME="${JOB_NAME:-jiaxicao-hybrid-2node-full}"
...
EXP_TAG="${EXP_TAG:-qwen35_9b_cc_ags_2node_hybrid_c64_t45}"
...
# Submit-batch size (waves of StartSandbox); true in-flight cap is SLIME_CC_AGENT_CONCURRENCY=64 in Job env.
...
if [[ -z "${WANDB_GROUP:-}" \
   ...
   || "${WANDB_GROUP}" == "qwen35_9b_cc_ags_1node_hybrid" \
   || "${WANDB_GROUP}" == "qwen35_9b_cc_ags_2node_hybrid_full" ]]; then
  WANDB_GROUP="${EXP_TAG}"
fi
...
# Refuse accidental overwrite of the destination checkpoint directory.
...
export STEP_GRPO_HYBRID_K STEP_GRPO_BRANCH_SUBMIT_BATCH STEP_GRPO_BRANCH_CONCURRENCY STEP_GRPO_PPL_CLIP STEP_GRPO_FILTER STEP_GRPO_BRANCH_LOSS_WEIGHT STEP_GRPO_STAGE2_LOSS_SCOPE
...
export SLIME_CC_TIME_BUDGET_SEC SLIME_CC_AGENT_CONCURRENCY
export SLIME_CC_TOOL_LOOP_PENALTY SLIME_CC_TIMEOUT_OUTCOME_REWARD

RENDERED="$(mktemp)"
envsubst '${JOB_NAME} ${WORKLOAD_LABEL} ${K8S_NAMESPACE} ${IMAGE_URI} ${SLIME_DIR} ${HF_CHECKPOINT} ${REF_MODEL_PATH} ${PROMPT_DATA} ${EVAL_DATA} ${EXP_TAG} ${LOG_DIR} ${LOAD_PATH} ${SAVE_PATH} ${RUN_ROOT} ${LOAD_CKPT_STEP} ${RESUME_DEBUG_ROLLOUT_DATA} ${PHASE} ${AGS_SECRET_NAME} ${SLIME_ADAPTER_PUBLIC_URL} ${SLIME_AGENT_AGS_TOOL_ID} ${WANDB_KEY} ${WANDB_PROJECT} ${WANDB_GROUP} ${WANDB_TEAM} ${WANDB_FORK_FROM} ${WANDB_RESUME_FROM} ${WANDB_RUN_ID} ${WANDB_RESUME} ${STEP_GRPO_HYBRID_K} ${STEP_GRPO_BRANCH_SUBMIT_BATCH} ${STEP_GRPO_BRANCH_CONCURRENCY} ${STEP_GRPO_PPL_CLIP} ${STEP_GRPO_FILTER} ${STEP_GRPO_BRANCH_LOSS_WEIGHT} ${STEP_GRPO_STAGE2_LOSS_SCOPE} ${STEP_GRPO_BUNDLE_DIR} ${NUM_ROLLOUT} ${SAVE_INTERVAL} ${GLOBAL_BATCH_SIZE} ${ROLLOUT_BATCH_SIZE} ${ACTOR_NUM_NODES} ${WORKER_REPLICAS} ${ACTOR_NUM_GPUS_PER_NODE} ${ROLLOUT_NUM_GPUS} ${SLIME_CC_TIME_BUDGET_SEC} ${SLIME_CC_AGENT_CONCURRENCY} ${SLIME_CC_TOOL_LOOP_PENALTY} ${SLIME_CC_TIMEOUT_OUTCOME_REWARD}' \
  < "${TEMPLATE}" > "${RENDERED}"
...
echo "    hybrid K=${STEP_GRPO_HYBRID_K} filter=${STEP_GRPO_FILTER} lambda=${STEP_GRPO_BRANCH_LOSS_WEIGHT} stage2_loss_scope=${STEP_GRPO_STAGE2_LOSS_SCOPE} submit_batch=${STEP_GRPO_BRANCH_SUBMIT_BATCH}"
echo "    agent_budget=${SLIME_CC_TIME_BUDGET_SEC}s agent_concurrency=${SLIME_CC_AGENT_CONCURRENCY}"
