...
RENDERED="$(mktemp)"
envsubst '${JOB_NAME} ${WORKLOAD_LABEL} ${K8S_NAMESPACE} ${IMAGE_URI} ${SLIME_DIR} ${HF_CHECKPOINT} ${REF_MODEL_PATH} ${PROMPT_DATA} ${EVAL_DATA} ${EXP_TAG} ${LOG_DIR} ${LOAD_PATH} ${SAVE_PATH} ${RUN_ROOT} ${LOAD_CKPT_STEP} ${RESUME_DEBUG_ROLLOUT_DATA} ${PHASE} ${AGS_SECRET_NAME} ${SLIME_ADAPTER_PUBLIC_URL} ${SLIME_AGENT_AGS_TOOL_ID} ${WANDB_KEY} ${WANDB_PROJECT} ${WANDB_GROUP} ${WANDB_TEAM} ${WANDB_RESUME_FROM} ${WANDB_RUN_ID} ${WANDB_RESUME} ${STEP_GRPO_HYBRID_K} ${STEP_GRPO_BRANCH_SUBMIT_BATCH} ${STEP_GRPO_BRANCH_CONCURRENCY} ${STEP_GRPO_PPL_CLIP} ${STEP_GRPO_FILTER} ${STEP_GRPO_BRANCH_LOSS_WEIGHT} ${STEP_GRPO_STAGE2_LOSS_SCOPE} ${STEP_GRPO_BUNDLE_DIR} ${NUM_ROLLOUT} ${SAVE_INTERVAL} ${GLOBAL_BATCH_SIZE} ${ROLLOUT_BATCH_SIZE} ${ACTOR_NUM_NODES} ${WORKER_REPLICAS} ${ACTOR_NUM_GPUS_PER_NODE} ${ROLLOUT_NUM_GPUS} ${SLIME_CC_TIME_BUDGET_SEC} ${SLIME_CC_AGENT_CONCURRENCY} ${SLIME_CC_TOOL_LOOP_PENALTY} ${SLIME_CC_TIMEOUT_OUTCOME_REWARD}' \
  < "${TEMPLATE}" > "${RENDERED}"
...
echo "    WANDB resume_from=${WANDB_RESUME_FROM:-none} run_id=${WANDB_RUN_ID:-new}"
