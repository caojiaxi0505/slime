#!/usr/bin/env bash
# Fresh Hybrid full-continuation 8-GPU run from step0 for 23 rollout steps.
# Includes workspace-metadata readfix: external Read paths are audit-only.
...
export JOB_NAME="${JOB_NAME:-jiaxicao-hybrid-s0-fullcont-s23-readfix}"
export WORKLOAD_LABEL="${WORKLOAD_LABEL:-${JOB_NAME}}"
export INGRESS_NAME="${INGRESS_NAME:-${JOB_NAME}-adapter}"

export EXP_TAG="${EXP_TAG:-qwen35_9b_cc_ags_hybrid_s0_fullcont_s23_swe_8gpu_readfix_v1}"
