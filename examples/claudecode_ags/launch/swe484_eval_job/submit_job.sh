...
Useful env:
  ...
  SLIME_AGENT_AGS_SWE_REX_ROOT expected SWE-ReX src directory; startup fails on mismatch
...
SLIME_AGENT_AGS_SWE_REX_ROOT="${SLIME_AGENT_AGS_SWE_REX_ROOT:-/mnt/sn-007/jiaxicao/code/SWE-ReX/src}"
...
for p in "${SLIME_DIR}" "${HF_CHECKPOINT}" "${REF_MODEL_PATH}" "${PROMPT_DATA}" "${EVAL_DATA}" "${LOAD_PATH}" "${SLIME_AGENT_AGS_SWE_REX_ROOT}"; do
  if [[ ! -e "${p}" ]]; then
...
export SLIME_AGENT_AGS_BOOT_TIMEOUT_SEC SLIME_AGENT_AGS_SWE_REX_ROOT
...
echo "    SWE-ReX root: ${SLIME_AGENT_AGS_SWE_REX_ROOT}"
