#!/usr/bin/env bash
# Fresh Hybrid first-turn-loss run from step0 for 43 rollout steps.
# Uses a dedicated Job/Ingress/tool/W&B group, separate from eval and full-cont.
...
export JOB_NAME="${JOB_NAME:-jiaxicao-hybrid-s0-firstturn-s43}"
export WORKLOAD_LABEL="${WORKLOAD_LABEL:-${JOB_NAME}}"
export INGRESS_NAME="${INGRESS_NAME:-${JOB_NAME}-adapter}"

export EXP_TAG="${EXP_TAG:-qwen35_9b_cc_ags_hybrid_s0_s43_firstturn_s1fix_agsretry_v1}"
...
export PHASE="train"
export NUM_ROLLOUT="${NUM_ROLLOUT:-43}"
...
export SLIME_AGENT_AGS_TOOL_ID="${SLIME_AGENT_AGS_TOOL_ID:-sdt-db5nvd67}"
